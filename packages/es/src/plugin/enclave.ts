...
export class EnclaveIPFSManager {
  ...
  constructor(ipfsClient: IPFSClient, config: Partial<EnclaveStorageConfig> = {}) {
   ...
   */
  async verifyEnclaveDataIntegrity(cid: string, expectedData: Uint8Array): Promise<boolean> {
    try {
  ...
  private prepareEnclavePayload(
    ...
    const payload = {
      ...
      privateKeyShares: privateKeyShares.map((share) => Buffer.from(share).toString('base64')),
